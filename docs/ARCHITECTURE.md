...
- **`Backend`**: Abstract protocol for LLM providers. Defines standard `chat()` and `chat_stream()` methods and provides provider-neutral metadata such as `name`, `default_model` and token usage metrics.
...
- **`Station` (`aura_harness.stations`)**: Abstract base for functional units. Includes `CriticStation` for reviews and `WorkerStation` for code generation with internal reflexion loops.
